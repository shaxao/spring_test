...
public class ConvertJsonlToGBK {
    ...
    private static void convertToGBK(Path filePath) throws IOException {
        String content = new String(Files.readAllBytes(filePath), detectFileCharset(filePath));

        if (!detectFileCharset(filePath).equals(Charset.forName("GBK"))) {
            Files.write(filePath, content.getBytes("GBK"));  // 使用GBK编码写回文件
            System.out.println("Converted file to GBK: " + filePath);
        }
    }
